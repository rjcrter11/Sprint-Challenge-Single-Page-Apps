...
> Answer: Routes in client side routing are handled by the Javascript that is loaded on the page, so when a link is clicked, the URL changes, but the request to the server is prevented. Because of this, the whole page won’t refresh—some elements in the page just change. Some of the advantages to this are that routing between views is generally faster, and smooth transitions an animations are easier to implement.
